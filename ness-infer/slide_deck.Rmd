...
pkgs <- c("tidyverse", "moderndive", "infer", "gapminder",
          "nycflights13", "fivethirtyeight")
...
`r gif_link("https://infer-dev.netlify.com", "img/infer_gnome.jpg", 380)`
...
```{r}
null_distn %>% 
  visualize(bins = 10, obs_stat = obs_diff, direction = "right")
```
...
yawn_myth %>%
  ...
  visualize(method = "theoretical", obs_stat = obs_diff,
            direction = "right")
```
...
yawn_myth %>%
  ...
  visualize(method = "both", bins = 10, obs_stat = obs_diff,
            direction = "right")
```
...
- Improve speed and space by shifting to list-columns
...
- Contact me: [Email](mailto:chester@datacamp.com) or [Twitter](https://twitter.com/old_man_chester)
